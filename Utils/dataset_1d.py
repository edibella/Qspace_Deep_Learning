...
class Dataset_1D(Dataset):
    def __init__(self, data_path, data_type = 'train', mask = True, shuffle = True, split = 1):
        super(Dataset_1D, self).__init__()
        ...
        y_means = np.array(h5py.File(data_path+'y_train_means.h5','r')['y_means'])
        y /= np.exp(np.floor(np.log(y_means))) 
        self.y = y
