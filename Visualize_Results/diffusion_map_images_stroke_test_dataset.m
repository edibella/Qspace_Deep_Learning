...
hfig = figure('unit','pixel','position',[1 1 11*ny+offy+offy_t 5*nx+offx+offx_t]);
...
for i = 0:10
    ...
    ax0 = axes('parent',hfig,'unit','pixel','position',[offy+i*ny 4*nx+offx ny nx]);
    ...
    ax0_bx = axes('parent',hfig,'unit','pixel','position',[offy+i*ny 4*nx+offx round(ny/2.5) round(nx/2.5)]);
    ...
    M = max(img0(:));
    colorbar(ax_cbar,'south','fontsize',ff_small);colormap(ax_cbar,'gray');caxis(ax_cbar,[0 M]);axis(ax_cbar,'off')
    
    imagesc(rot90(img0),'parent',ax0);caxis(ax0,[0 M]);colormap(ax0,'gray');
    title(ax0,maps_name{i+1},'interpreter','latex','fontsize',ff);
    set(ax0,'XTickLabel','');set(ax0,'YTickLabel','');
    rectangle(ax0,'Position',box_pos,'LineWidth',1,'EdgeColor','g');
    
    imagesc(rot90(img0_bx),'parent',ax0_bx);caxis(ax0_bx,[0 M]);colormap(ax0_bx,'gray');
    set(ax0_bx,'XTickLabel','');set(ax0_bx,'YTickLabel','');set(ax0_bx,'XColor','g');set(ax0_bx,'YColor','g');
    ...
    imagesc(rot90(img1),'parent',ax1);caxis(ax1,[0 M]);colormap(ax1,'gray');
    set(ax1,'XTickLabel','');set(ax1,'YTickLabel','');
    imagesc(rot90(img1_bx),'parent',ax1_bx);caxis(ax1_bx,[0 M]);colormap(ax1_bx,'gray');
    ...
    imagesc(rot90(img2),'parent',ax2);caxis(ax2,[0 M]);colormap(ax2,'gray');
    set(ax2,'XTickLabel','');set(ax2,'YTickLabel','');
    imagesc(rot90(img2_bx),'parent',ax2_bx);caxis(ax2_bx,[0 M]);colormap(ax2_bx,'gray');
    ...
    imagesc(rot90(img3),'parent',ax3);caxis(ax3,[0 M]);colormap(ax3,'gray');
    set(ax3,'XTickLabel','');set(ax3,'YTickLabel','');
    imagesc(rot90(img3_bx),'parent',ax3_bx);caxis(ax3_bx,[0 M]);colormap(ax3_bx,'gray');
    ...
    imagesc(rot90(img4),'parent',ax4);caxis(ax4,[0 M]);colormap(ax4,'gray');
    set(ax4,'XTickLabel','');set(ax4,'YTickLabel','');
    imagesc(rot90(img4_bx),'parent',ax4_bx);caxis(ax4_bx,[0 M]);colormap(ax4_bx,'gray');
    set(ax4_bx,'XTickLabel','');set(ax4_bx,'YTickLabel','');set(ax4_bx,'XColor','g');set(ax4_bx,'YColor','g');
        
end

ax_lb0 = axes('parent',hfig,'unit','pixel','position',[0 4*nx+offx offy nx]);
...
back = ones(nx,offy);
imagesc(back,'parent',ax_lb0);caxis(ax_lb0,[0 1]);colormap(ax_lb0,'gray');text(ax_lb0,round(offy/2),round(nx/2),{'\textbf{REF}'},'color','black','fontsize',ff,'HorizontalAlignment','center','Interpreter','latex');axis(ax_lb0,'off')
